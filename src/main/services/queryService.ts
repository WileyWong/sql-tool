 ...
 * SQL 查询服务
 *
 * 承载 ipc/query.ts 中的会话编排、执行、序列化逻辑。
 */
 ...
 * 处理 BigInt、Buffer、Uint8Array 等特殊类型。纯函数，便于单元测试。
 */
 ...
 */
export async function ensureSession(tabId: string, connectionId: string, database?: string): Promise<void> {
...
export async function execute(data: {
  ...
  try {
    ...
    const sessionManager = DriverFactory.getSessionManager(getConnectionDbType(data.connectionId))
    ...
    return {
      success: false,
      results: [{ type: 'error', code: 'E4000', message: errMessage(error, '执行失败') }],
    }
  }
}

/** 取消查询 */
export async function cancel(data: { connectionId: string; tabId: string }) {
  try {
    const sessionManager = DriverFactory.getSessionManager(getConnectionDbType(data.connectionId))
...
/** 获取执行计划 */
export async function explain(data: { connectionId: string; tabId: string; sql: string; database?: string }) {
  try {
    await ensureSession(data.tabId, data.connectionId, data.database)
    const sessionManager = DriverFactory.getSessionManager(getConnectionDbType(data.connectionId))
    ...
    return {
      success: false,
      error: { type: 'error', code: 'E4000', message: errMessage(error, '获取执行计划失败') },
    }
  }
}

/** 批量执行 SQL */
export async function executeBatch(data: { connectionId: string; tabId: string; sqls: string[]; database?: string }) {
  try {
    await ensureSession(data.tabId, data.connectionId, data.database)
    const sessionManager = DriverFactory.getSessionManager(getConnectionDbType(data.connectionId))
    return await sessionManager.executeBatch(data.tabId, data.sqls)
  } catch (error: unknown) {
    return { success: false, message: errMessage(error, '批量执行失败') }
  }
}
