...
import { LOCALE, SUPPORT_LANGUAGES, Languages } from '../config';

/**
 * 解析描述或原因，转换 <, >, \n 等字符串
 * @param str 需要解析的文本
 */
export const parseDescription = function parseDescription(str: string) {
...
/** 翻译 */
export const t = function t(str: string) {
  const language = getLanguage();
  return LOCALE[language][str] || str;
};

/** 从 query 和 cookies 获取 language */
export const getLanguage = function getLanguage(): Languages {
  let language: any;
  ...
  if (!SUPPORT_LANGUAGES.includes(language)) {
    ({ language } = cookie.parse(document.cookie));
    // 非法的 cookieLanguage
    if (!SUPPORT_LANGUAGES.includes(language)) {
...
export const getQuery = function getQuery() {
  return querystring.parse(location.search.replace(/^\?/, ''));
};

const setCookie = function setCookie(name: string, value: string, days = 365) {
...
/** 生成新的 url */
export const newUrl = function newUrl({ path, query }: UrlOptions) {
...
export const replaceUrl = function replaceUrl(url: string, title?: string) {
  history.replaceState({}, title || document.title, url);
};

/** 若为空则返回默认值，可提供取值范围 */
export const defaultTo = function defaultTo<T>(
  source: any,
  defaultValue: T,
  avaliableValues?: T[],
): T {
