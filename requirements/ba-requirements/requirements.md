...
#### 3.3 代码格式化
**触发方式**：快捷键 Shift+Alt+F 或右键菜单"格式化文档"
**格式化规则**：
...
#### 3.4 悬浮提示
**触发方式**：鼠标悬停在表名、字段名、函数名上
**提示内容**：
- 表名：显示表注释、字段数量、所属数据库
- 字段名：显示字段类型、是否可空、默认值、字段注释
- 函数名：显示函数签名、参数说明、返回值类型
...
1. **代码格式化 (3.3)**
   - 触发方式：快捷键 Shift+Alt+F 或右键菜单
   - 格式化规则：关键字大写、适当缩进、操作符空格
...
| 决策项 | 决议 |
|--------|------|
| 智能功能实现方案 | 采用 C2 方案：monaco-languageclient + LSP 架构 |
| SQL 解析器 | sql-parser-cst |
| 代码格式化库 | sql-formatter |
| Language Server 运行位置 | Electron Main 进程 |

**技术设计文档**: `requirements/tech-design/sql-language-server.md`
