...
**功能描述**: 左侧数据库树组件支持节点级别的过滤，鼠标悬停时显示过滤按钮，支持对表、视图、函数等子节点进行过滤。

**用户场景**: 用户在数据库节点下有大量表时，通过过滤快速找到目标表、视图或函数。

**输入/输出**: 
- 输入：鼠标悬停数据库节点，点击过滤按钮，输入过滤关键字
- 输出：匹配的子节点（表、视图、函数等）

**业务规则**: 
- 鼠标悬停数据库节点时显示"过滤"文字按钮
- 点击后原地变为单行文本框
- 回车键执行过滤，ESC键退出过滤模式
- 过滤所有子节点类型（表、视图、函数、存储过程等）
- 每个数据库节点独立维护过滤状态
...
**验收标准**:
- [x] 正常场景: 悬停显示过滤按钮，输入后过滤子节点
- [x] 边界条件: 在不同数据库节点间切换时，各自保持独立的过滤状态
- [x] 异常情况: 输入无效字符时正常处理，不影响系统稳定性

**确认项**:
- [CLARIFIED] 过滤状态的持久性
  - 决议: 每个树节点独立维护过滤状态，不同树节点分别过滤
  - 理由: 每个数据库节点的过滤需求不同，独立过滤更符合使用习惯
  - 验收标准: 每个数据库节点都可以独立设置和保持过滤条件
